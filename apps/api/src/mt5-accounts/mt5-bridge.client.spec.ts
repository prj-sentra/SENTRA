...
describe('Mt5BridgeClient', () => {
  ...
  it('uses bounded authenticated I/O and accepts an exact identity echo', async () => {
    global.fetch = jest.fn().mockResolvedValue(new Response(JSON.stringify({
      server: 'broker-live', accountLogin: 123, actions: { rawText: 'sync', source: 'api', actions: [] },
    }), { status: 200 })) as typeof fetch;
    const client = new Mt5BridgeClient();

    await expect(client.sync({ server: 'broker-live', accountLogin: 123, password: 'write-only' }))
  ...
  it('rejects a mismatched bridge identity', async () => {
    global.fetch = jest.fn().mockResolvedValue(new Response(JSON.stringify({
      server: 'attacker', accountLogin: 999, actions: { rawText: 'sync', source: 'api', actions: [] },
    }), { status: 200 })) as typeof fetch;
