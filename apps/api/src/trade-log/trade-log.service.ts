...
export class TradeLogService {
  ...
  async getStats(ownerId: string, query: TradeStatsQuery): Promise<TradeStatsResponse> {
    ...
    const dimensions: TradeStatsDimension[] = ['symbol', 'side', 'exitReason', 'entryWeekday', 'session', 'baseTimeframe', 'bollingerBandCount', 'bollingerDirection', 'executionEvaluation', 'violationFlags', 'holdDuration', 'analysisCompleteness'];
  ...
  private validateStatsQuery(query: TradeStatsQuery): TradeStatsQuery {
    ...
    for (const key of ['symbols', 'sides', 'sessions', 'baseTimeframes', 'outcomes', 'evaluations', 'violations', 'exitReasons', 'entryWeekdays', 'bollingerBandCounts', 'bollingerDirections', 'analysisCompleteness', 'holdDurationBands'] as const) {
    ...
    if (normalized.unit !== undefined && !['campaign', 'trade'].includes(normalized.unit) || !valid(normalized.sides, ['long', 'short']) || !valid(normalized.sessions, ['asia', 'london', 'new-york', 'off-session']) || !valid(normalized.outcomes, ['win', 'loss', 'breakeven', 'unclassified']) || !valid(normalized.evaluations, ['as_planned', 'plan_violated']) || !valid(normalized.bollingerBandCounts, ['one_band', 'two_band']) || !valid(normalized.bollingerDirections, ['normal', 'reverse', 'chase']) || !valid(normalized.analysisCompleteness, ['complete', 'incomplete']) || (normalized.rowDimension !== undefined && !['symbol', 'side', 'exitReason', 'entryWeekday', 'session', 'baseTimeframe', 'bollingerBandCount', 'bollingerDirection', 'executionEvaluation', 'violationFlags', 'holdDuration', 'analysisCompleteness'].includes(normalized.rowDimension)) || (normalized.columnDimension !== undefined && !['symbol', 'side', 'exitReason', 'entryWeekday', 'session', 'baseTimeframe', 'bollingerBandCount', 'bollingerDirection', 'executionEvaluation', 'violationFlags', 'holdDuration', 'analysisCompleteness'].includes(normalized.columnDimension))) throw new BadRequestException('Stats query is invalid');
