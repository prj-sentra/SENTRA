...
async function main(): Promise<void> {
  ...
  try {
    ...
    await client.query(`
      INSERT INTO "app_users" ("id", "username", "normalized_username", "password_hash", "updated_at") VALUES ('00000000-0000-0000-0000-000000000001', 'verify-user', 'verify-user', 'hash', CURRENT_TIMESTAMP);
      INSERT INTO "mt5_accounts" ("id", "owner_id", "nickname", "server", "canonical_server", "account_login", "credential_ciphertext", "credential_iv", "credential_tag", "updated_at") VALUES ('verify-account', '00000000-0000-0000-0000-000000000001', 'Verifier', 'broker', 'broker', 42, '\\x00', '\\x00', '\\x00', CURRENT_TIMESTAMP);
      INSERT INTO "trades" ("id", "symbol", "side", "status", "owner_id", "mt5_account_id", "updatedAt") VALUES ('verify-trade', 'EURUSD', 'long', 'closed', '00000000-0000-0000-0000-000000000001', 'verify-account', CURRENT_TIMESTAMP);
      INSERT INTO "trade_campaigns" ("id", "root_trade_id", "trading_date", "owner_id", "mt5_account_id", "updated_at") VALUES ('verify-campaign', 'verify-trade', CURRENT_DATE, '00000000-0000-0000-0000-000000000001', 'verify-account', CURRENT_TIMESTAMP);
    ...
    await expectFailure(() => client.query(`UPDATE "trade_excursion_results" SET "status"='FAILED', "failure_reason"='failed', "success_calculation_version"=NULL, "success_input_fingerprint"=NULL, "last_succeeded_at"=NULL, "mfe_price"=1 WHERE "trade_id"='verify-trade'`), 'trade_excursion_results_failed_metrics_check');
    await expectFailure(() => client.query(`INSERT INTO "excursion_work_items" ("scope", "target_id", "trade_id", "campaign_id", "account_id", "base_input_fingerprint", "reason", "updated_at") VALUES ('TRADE', 'verify-trade', NULL, NULL, 'verify-account', 'input', 'verify', CURRENT_TIMESTAMP)`), 'excursion_work_items_scope_target_check');
    await client.query(`DELETE FROM "trades" WHERE "id"='verify-trade'`);
