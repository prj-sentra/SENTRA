...
BEGIN
  ...
  FOR i IN 0..9 LOOP INSERT INTO "trade_campaign_images" ("id","campaign_id","position","file_name","mime_type","byte_size","content_sha256","width","height","created_at","updated_at") VALUES (md5(i::text||trade_id),campaign_id,i,i||'.png','image/png',i+1,repeat('0',64),1,1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP); END LOOP;
  UPDATE "trade_campaign_images" image
  SET "position" = CASE image."position" WHEN 0 THEN 1 WHEN 1 THEN 0 ELSE image."position" END
  WHERE image."campaign_id"=campaign_id;
  IF (SELECT image."position" FROM "trade_campaign_images" image WHERE image."campaign_id"=campaign_id AND image."id"=md5('0'||trade_id)) <> 1
    OR (SELECT image."position" FROM "trade_campaign_images" image WHERE image."campaign_id"=campaign_id AND image."id"=md5('1'||trade_id)) <> 0
    THEN RAISE EXCEPTION 'gallery reorder failed'; END IF;
  BEGIN INSERT INTO "trade_campaign_images" ("id","campaign_id","position","file_name","mime_type","byte_size","content_sha256","width","height","created_at","updated_at") VALUES (md5('overflow'||trade_id),campaign_id,10,'overflow.png','image/png',1,repeat('0',64),1,1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP); RAISE EXCEPTION 'accepted >10 images'; EXCEPTION WHEN check_violation THEN NULL; END;
