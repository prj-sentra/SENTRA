...
FROM (SELECT DISTINCT lower(regexp_replace(btrim("server"), '[[:space:]]+', ' ', 'g')) s, "account_login" l FROM (SELECT "server", "account_login" FROM "mt5_deals" UNION ALL SELECT "server", "account_login" FROM "mt5_orders" UNION ALL SELECT "server", "account_login" FROM "mt5_sync_status" UNION ALL SELECT "mt5_server", "mt5_account_login" FROM "trades" WHERE "mt5_server" IS NOT NULL AND "mt5_account_login" IS NOT NULL) identities) i;
...
UPDATE "trades" t SET "owner_id" = '00000000-0000-0000-0000-000000000001', "mt5_account_id" = a."id" FROM "mt5_accounts" a WHERE a."canonical_server" = lower(regexp_replace(btrim(t."mt5_server"), '[[:space:]]+', ' ', 'g')) AND a."account_login" = t."mt5_account_login";
...
UPDATE "mt5_deals" d SET "server" = a."canonical_server", "account_id" = a."id" FROM "mt5_accounts" a WHERE a."canonical_server" = lower(regexp_replace(btrim(d."server"), '[[:space:]]+', ' ', 'g')) AND a."account_login" = d."account_login";
...
DO $$ BEGIN
  ...
  IF EXISTS (SELECT 1 FROM "trade_chart_images") AND to_regclass('public.legacy_trade_chart_image_file_manifest') IS NULL THEN RAISE EXCEPTION 'legacy image file manifest required'; END IF;
END $$;
...
CREATE TABLE "trade_campaign_images" ("id" TEXT PRIMARY KEY, "campaign_id" TEXT NOT NULL, "position" INTEGER NOT NULL, "file_name" TEXT NOT NULL, "mime_type" TEXT NOT NULL, "byte_size" INTEGER NOT NULL, "width" INTEGER NOT NULL, "height" INTEGER NOT NULL, "original_name" TEXT, "created_at" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP, "updated_at" TIMESTAMP(3) NOT NULL, CHECK ("position" >= 0 AND "position" < 10), FOREIGN KEY ("campaign_id") REFERENCES "trade_campaigns"("id") ON DELETE CASCADE);
CREATE UNIQUE INDEX "trade_campaign_images_campaign_id_position_key" ON "trade_campaign_images"("campaign_id", "position");
INSERT INTO "trade_campaign_images" SELECT i."id", im.campaign_id, row_number() OVER (PARTITION BY im.campaign_id ORDER BY CASE WHEN c."root_trade_id"=i."trade_id" THEN 0 ELSE 1 END, i."created_at", i."id")-1, i."file_name", i."mime_type", i."byte_size", i."width", i."height", i."original_name", i."created_at", i."updated_at" FROM "trade_chart_images" i JOIN image_map im ON im.image_id=i."id" JOIN "trade_campaigns" c ON c."id"=im.campaign_id;
DO $$ BEGIN
  ...
  IF EXISTS (SELECT 1 FROM "trade_chart_images" s LEFT JOIN "trade_campaign_images" d ON d."id"=s."id" WHERE d."id" IS NULL OR (s."file_name",s."mime_type",s."byte_size",s."width",s."height",s."original_name",s."created_at",s."updated_at") IS DISTINCT FROM (d."file_name",d."mime_type",d."byte_size",d."width",d."height",d."original_name",d."created_at",d."updated_at")) THEN RAISE EXCEPTION 'image metadata reconciliation failed'; END IF;
END $$;
