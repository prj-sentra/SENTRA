...
export const TradeAnalysisEditor = forwardRef<HTMLFormElement, { trade: TradeRecord; campaign?: TradeCampaign; scope?: 'execution' | 'campaign'; onSave: (tradeId: string, patch: PatchTradeAnalysisRequest) => Promise<void>; onSaveCampaign?: (campaignId: string, patch: PatchTradeCampaignAnalysisRequest) => Promise<void> }>(({ trade, campaign, scope = 'execution', onSave, onSaveCampaign }, ref) => {
  ...
  return <form ref={ref} className="detail-editor analysis-editor" onSubmit={(event) => void submit(event)} onReset={() => { setDraft(makeDraft()); setError(null); }}>
    ...
    {scope === 'execution' ? <>
      ...
      <Row title="볼린저밴드" help="이 실행의 볼린저밴드 터치 및 방향을 기록합니다."><label><span>볼린저밴드 터치</span><select value={draft.bollingerBandCount ?? ''} onChange={(e) => patch({ bollingerBandCount: (e.target.value || null) as Draft['bollingerBandCount'], ...(!e.target.value || e.target.value === 'no_touch' ? { bollingerDirection: null } : {}) })}><option value="">미평가</option><option value="no_touch">터치 안함</option><option value="one_band">원볼</option><option value="two_band">투볼</option></select></label><label><span>볼린저밴드 추세</span><select value={draft.bollingerDirection ?? ''} disabled={!draft.bollingerBandCount || draft.bollingerBandCount === 'no_touch'} onChange={(e) => patch({ bollingerDirection: (e.target.value || null) as Draft['bollingerDirection'] })}><option value="">선택</option><option value="normal">정볼</option><option value="reverse">역볼</option><option value="chase">추볼</option></select></label></Row>
      <Row className="execution-evaluation-row" title="실행 평가" help="이 진입이 사전에 세운 계획대로 실행되었는지 평가합니다.">
    ...
    {scope === 'campaign' ? <Row className="economic-indicator-row" title="경제지표" help="매매 전체에 영향을 준 경제지표와 발표 시간 및 결과를 기록합니다.">
