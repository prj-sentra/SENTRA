...
describe('StatsPage', () => {
  it('renders the selected-unit excursion summary and distributions', async () => {
    const request = vi.fn(() => Promise.resolve({ ...stats, excursions: { unit: 'campaign', families: [{ family: 'campaign_price', status: { success: 2, stale: 1, failed: 1, unsupported: 1, missing: 3 }, price: { mfe: { sampleCount: 2, mean: 3, bins: [] }, mae: { sampleCount: 2, mean: -2, bins: [] } }, percent: { mfe: { sampleCount: 2, bins: [] }, mae: { sampleCount: 2, bins: [] } }, counts: { eligibleSuccessCount: 2, heterogeneousUnavailableCount: 1 } }, { family: 'campaign_unrealized_pnl', status: { success: 2, stale: 0, failed: 0, unsupported: 0, missing: 0 }, unrealizedPnl: { mfe: { sampleCount: 2, mean: 10, median: 9.65, bins: [{ min: 0, max: 10, includeMax: true, count: 2 }] }, mae: { sampleCount: 2, mean: -5, median: -8.51, bins: [{ min: -20, max: -10, includeMax: false, count: 1 }, { min: -10, max: 0, includeMax: true, count: 1 }] } }, r: { mfe: { sampleCount: 0, bins: [] }, mae: { sampleCount: 0, bins: [] } }, captureRate: { sampleCount: 1, mean: 50, bins: [] }, management: { accountCurrency: 'USD', eligibleSuccessCount: 2, opportunityReversal: { count: 1, rate: 50 }, riskDominant: { count: 1, rate: 50 }, profitableCapture: { eligibleCount: 1, distribution: { sampleCount: 1, mean: 76.23, median: 76.23, q1: 76.23, q3: 76.23, bins: [{ min: 76.23, max: 76.23, includeMax: true, count: 1 }] }, belowFiftyCount: 0, belowFiftyRate: 0, bands: [{ key: 'opportunity_loss', count: 1 }, { key: 'under_25', count: 0 }, { key: '25_50', count: 0 }, { key: '50_75', count: 0 }, { key: '75_100', count: 1 }, { key: '100_plus', count: 0 }] }, }, counts: { eligibleSuccessCount: 2, riskUnavailableCount: 0, captureEligibleCount: 1, valuationUnavailableCount: 0 } }] } } as TradeStatsResponse));
    page(request);
    expect(await screen.findByText('시장 진행 분석')).toBeInTheDocument();
    expect(screen.getByText('최대 수익 기회 중앙값')).toBeInTheDocument();
    expect(screen.getByText('최대 손실 위험 중앙값')).toBeInTheDocument();
    expect(screen.getByText('수익 거래 실현률 중앙값')).toBeInTheDocument();
    expect(screen.getByText('데이터 품질 및 기술 통계')).toBeInTheDocument();
    expect(screen.getByText('계산 완료 2 / 2')).toBeInTheDocument();
    expect(screen.getAllByText('9.65 USD').length).toBeGreaterThan(0);
    expect(screen.getByText('76.23%')).toBeInTheDocument();
    expect(screen.getByText('수익 기회 후 손실 전환')).toBeInTheDocument();
    expect(screen.getByLabelText('최대 수익 기회와 최대 손실 위험 분포')).toBeInTheDocument();
    expect(screen.getByLabelText('수익 실현률 구간')).toBeInTheDocument();
    const riskBars = screen.getByRole('img', { name: /최대 손실 위험 분포/ }).querySelectorAll('span');
    expect(riskBars[0]).toHaveAttribute('title', '-20~-10: 1건');
    expect(riskBars[1]).toHaveAttribute('title', '-10~0: 1건');
