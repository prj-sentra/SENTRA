...
function AveragePerformance({ overview }: { overview: TradeStatsOverview }) { return <article className="average-performance" aria-label="평균 성과"><div className="average-term average-primary"><MetricLabel>거래당 평균 PnL</MetricLabel><strong>${format(overview.averageRealizedPnl)}</strong></div><div className="average-term"><MetricLabel tooltip="선택한 각 매매에 1 Lot으로 진입했다고 가정했을 때의 환산 PnL을 구한 뒤, 그 값들의 평균을 계산합니다.">거래당 평균 포인트</MetricLabel><strong>${format(overview.oneLotPnl)}</strong></div><div className="average-term"><MetricLabel>평균 수익 PnL</MetricLabel><strong className="is-profit">+${format(overview.averageWin === undefined ? undefined : Math.abs(overview.averageWin))}</strong></div><div className="average-term"><MetricLabel>평균 손실 PnL</MetricLabel><strong className="is-loss">-${format(overview.averageLoss === undefined ? undefined : Math.abs(overview.averageLoss))}</strong></div></article>; }
...
function Diagnostics({ diagnostics, excursions }: { diagnostics: TradeStatsResponse['diagnostics']; excursions?: TradeStatsResponse['excursions'] }) { const entries = [['시드 잔고', diagnostics.missingSeedCount, diagnostics.missingSeedIds], ['lot', diagnostics.missingLotsCount, diagnostics.missingLotsIds]] as const; const families = excursions?.unit === 'trade' ? excursions.families : excursions?.unit === 'campaign' ? excursions.families : []; const bins = (distribution: { sampleCount: number; bins: Array<{ min: number; max: number; count: number }> }) => distribution.sampleCount ? <p>분포 {distribution.bins.map((bin) => `${format(bin.min)}~${format(bin.max)}: ${bin.count}`).join(' · ')}</p> : <p className="muted">적격 성공 표본 없음</p>; return <><section className="detail-metric-group" aria-label="MFE MAE 통계">{families.map((family) => <div key={family.family}><h3>{family.family === 'trade' ? '진입 MFE/MAE' : family.family === 'campaign_price' ? '매매 가격 MFE/MAE' : '매매 미실현 PnL MFE/MAE'}</h3><p>성공 {family.status.success} · 오래됨 {family.status.stale} · 실패 {family.status.failed} · 미지원 {family.status.unsupported} · 누락 {family.status.missing}</p>{'price' in family && <><p>가격 MFE 평균 {format(family.price.mfe.mean)} / MAE 평균 {format(family.price.mae.mean)} · 표본 {family.price.mfe.sampleCount}</p>{bins(family.price.mfe)}</>}{'unrealizedPnl' in family && <><p>미실현 PnL MFE 평균 {format(family.unrealizedPnl.mfe.mean)} / MAE 평균 {format(family.unrealizedPnl.mae.mean)} · 표본 {family.unrealizedPnl.mfe.sampleCount}</p>{bins(family.unrealizedPnl.mfe)}</>}{'r' in family && <p>R 가능 {family.counts.eligibleSuccessCount - family.counts.riskUnavailableCount} · 위험금액 없음 {family.counts.riskUnavailableCount}</p>}{'captureRate' in family && <p>캡처율 평균 {format(family.captureRate.mean, '%')} · 가능 {family.counts.captureEligibleCount} · 불가 {family.counts.eligibleSuccessCount - family.counts.captureEligibleCount}</p>}</div>)}</section>{entries.filter(([, count]) => count > 0).map(([label, count, ids]) => <p key={label} className="error-banner" role="alert">{label} 누락/주의 {count}건: {ids.join(', ')}</p>)}</>; }
function Series({ series, granularity, setGranularity, unit }: { series: TradeStatsResponse['timeSeries']; granularity: TradeStatsGranularity; setGranularity: (value: TradeStatsGranularity) => void; unit: TradeStatsUnit }) {
  ...
  return <section id="dashboard-charts" className="stats-series dashboard-section">
    ...
    {active.points.length ? <section className="stats-chart-panel" role="tabpanel" aria-labelledby={`chart-type-${chartType}`}><h3>{selectedChart[1]}</h3><StatsLineChart points={active.points} value={selectedChart[2]} label="성과 추이" percent={selectedChart[3]} spacing={granularity === 'sequence' ? spacing : 'time'} /></section> : <p className="muted">선택한 기간의 시계열이 없습니다.</p>}
  </section>;
}
