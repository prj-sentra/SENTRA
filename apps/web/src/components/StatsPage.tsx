...
import { StatsTradingViewChart } from './StatsTradingViewChart';
  ...
  return <aside className={`dashboard-filter-sidebar${open ? ' is-open' : ''}`} aria-label="대시보드 공통 필터">
    ...
    {open ? <div className="dashboard-filter-expanded">
      ...
      <section className="dashboard-filter-dates">
        ...
        <label className="stats-through-latest">오늘<input type="checkbox" checked={throughLatest} onChange={(event) => { const checked = event.target.checked; setThroughLatest(checked); setFilters({ ...filters, to: checked ? today : filters.to ?? today }); }} /></label>
      </section>
...
function Diagnostics({ diagnostics }: { diagnostics: TradeStatsResponse['diagnostics'] }) { const entries = [['시드 잔고', diagnostics.missingSeedCount, diagnostics.missingSeedIds], ['lot', diagnostics.missingLotsCount, diagnostics.missingLotsIds]] as const; return <>{entries.filter(([, count]) => count > 0).map(([label, count, ids]) => <p key={label} className="error-banner" role="alert">{label} 누락/주의 {count}건: {ids.join(', ')}</p>)}</>; }
function Series({ series, granularity, setGranularity, unit }: { series: TradeStatsResponse['timeSeries']; granularity: TradeStatsGranularity; setGranularity: (value: TradeStatsGranularity) => void; unit: TradeStatsUnit }) { const active = series[granularity]; const unitLabel = unit === 'campaign' ? '매매' : '진입'; const labels = granularities.map(([value, label]) => [value, value === 'sequence' ? unitLabel : label] as const); const note = granularity === 'sequence' ? `실제 청산 시간 간격 · ${unitLabel}당 평균 ${format(active.activeBucketAverage)}` : `활성 기간 평균 ${format(active.activeBucketAverage)} · 달력 기간 평균 ${format(active.calendarBucketAverage)}. 진행 중인 현재 ${labels.find(([value]) => value === granularity)?.[1]} 기간은 완료 기간과 다를 수 있습니다.`; const proportionalTime = granularity === 'sequence'; return <section id="dashboard-charts" className="stats-series dashboard-section"><header><div className="stats-unit" role="tablist" aria-label="시계열 단위">{labels.map(([value, label]) => <button key={value} type="button" role="tab" aria-selected={granularity === value} className={granularity === value ? 'active' : ''} onClick={() => setGranularity(value)}>{label}</button>)}</div></header><p className="stats-note">{note}</p>{active.points.length ? <div className="stats-chart-stack"><section><h3>누적 실현 PnL</h3><StatsTradingViewChart points={active.points} value={(point) => point.equity} label="누적 실현 PnL TradingView 추이" proportionalTime={proportionalTime} /></section><section><h3>승률 변화</h3><StatsTradingViewChart points={active.points} value={(point) => point.winRate ?? 0} label="누적 승률 TradingView 추이" percent proportionalTime={proportionalTime} /></section></div> : <p className="muted">선택한 기간의 시계열이 없습니다.</p>}</section>; }
export function StatsPage({ accountId, request }: StatsPageProps) { const [stats, setStats] = useState<TradeStatsResponse | null>(null); const [unit, setUnit] = useState<TradeStatsUnit>('campaign'); const [filters, setFilters] = useState<Filters>({}); const [throughLatest, setThroughLatest] = useState(true); const [granularity, setGranularity] = useState<TradeStatsGranularity>('day'); const [error, setError] = useState(''); const requestVersion = useRef(0); const initializedAccount = useRef(''); const today = new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Seoul', year: 'numeric', month: '2-digit', day: '2-digit' }).format(new Date()); const loadStats = useCallback(async () => { const version = ++requestVersion.current; setError(''); try { const next = await request<TradeStatsResponse>(`/trade-log/stats?${query(accountId, unit, filters)}`); if (version !== requestVersion.current) return; setStats(next); if (initializedAccount.current !== accountId) { initializedAccount.current = accountId; const firstTradeDate = next.drilldown.reduce<string | undefined>((first, record) => !first || record.journalDate < first ? record.journalDate : first, undefined); setThroughLatest(true); setFilters({ from: firstTradeDate ?? today }); } } catch { if (version === requestVersion.current) setError('통계를 불러올 수 없습니다.'); } }, [accountId, filters, request, today, unit]); useEffect(() => { void loadStats(); }, [loadStats]); const drilldown = stats?.drilldown ?? []; if (!stats) return <p className="muted" role={error ? 'alert' : 'status'}>{error || '통계를 불러오는 중입니다…'}</p>; const overview = stats.overview; return <main className="stats-page"><section id="dashboard-period" className="dashboard-period-section dashboard-section"><h2 className="dashboard-section-title">기간별 성과</h2><FilterVisibilityProvider><DashboardFilterSidebar unit={unit} setUnit={setUnit} filters={filters} setFilters={setFilters} throughLatest={throughLatest} setThroughLatest={setThroughLatest} today={today} defaultFrom={stats.drilldown.reduce((first, record) => !first || record.journalDate < first ? record.journalDate : first, undefined as string | undefined)} options={stats.filterOptions ?? {}} /></FilterVisibilityProvider></section>{error && <p className="error-banner" role="alert">{error} 이전 결과를 표시합니다.</p>}<Diagnostics diagnostics={stats.diagnostics} /><section className="metric-grid" aria-label="핵심 성과 지표"><PnlHero overview={overview} /><AveragePerformance overview={overview} /></section><Series series={stats.timeSeries} granularity={granularity} setGranularity={setGranularity} unit={unit} /><section className="advanced-metric-grid" aria-label="고급 성과 지표"><TradeQuality overview={overview} /><StreakPerformance overview={overview} /><RiskAdjustedPerformance overview={overview} /><DrawdownPerformance drawdown={stats.drawdown} /></section><IntegratedPerformanceViewer selected={filters.groupDimensions ?? []} groups={stats.performanceGroups} /></main>; }
