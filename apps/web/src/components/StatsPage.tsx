...
function RiskAdjustedPerformance({ overview }: { overview: TradeStatsOverview }) { return <article className="detail-metric-group risk-performance" aria-label="위험 조정 성과"><h3>위험 조정 성과</h3><div><section><MetricLabel tooltip="각 거래의 손익을 해당 거래에서 감수한 초기 위험금액으로 나눈 R 값의 누적 합계입니다.">총 R</MetricLabel><strong>{format(overview.r.total ?? 0)}R</strong></section><section><MetricLabel tooltip="R을 계산할 수 있는 거래들의 평균 성과입니다. 예: +0.5R은 거래당 초기 위험의 절반만큼 평균 수익을 냈다는 뜻입니다.">평균 R</MetricLabel><strong>{format(overview.r.value ?? 0)}R</strong></section><section><MetricLabel tooltip="승리·손실·본전을 포함한 분류 가능 거래에서 거래당 기대되는 평균 R입니다.">기대 R</MetricLabel><strong>{format(overview.r.expectancy ?? 0)}R</strong></section></div></article>; }
function DrawdownPerformance({ drawdown }: { drawdown: TradeStatsResponse['drawdown'] }) { return <article className="detail-metric-group drawdown-performance" aria-label="최대 낙폭"><h3>최대 낙폭 (MDD)</h3><div><section><MetricLabel tooltip="누적 손익의 고점에서 이후 저점까지 발생한 가장 큰 금액 감소입니다.">금액 MDD</MetricLabel><strong>{format(drawdown.money ?? 0)}</strong></section><section><MetricLabel tooltip="최대 금액 낙폭을 당시 기준 잔고 대비 백분율로 표시한 값입니다.">비율 MDD</MetricLabel><strong>{format(drawdown.percent ?? 0, '%')}</strong></section><section><MetricLabel tooltip="최대 낙폭을 초기 위험 단위 R로 환산한 값입니다.">R MDD</MetricLabel><strong>{format(drawdown.r ?? 0)}R</strong></section></div></article>; }
...
function AveragePerformance({ overview }: { overview: TradeStatsOverview }) { return <article className="average-performance" aria-label="평균 성과"><div className="average-term average-primary"><MetricLabel>거래당 평균 PnL</MetricLabel><strong>${format(overview.averageRealizedPnl)}</strong></div><div className="average-term"><MetricLabel tooltip="선택한 각 매매에 1 Lot으로 진입했다고 가정했을 때의 환산 PnL을 구한 뒤, 그 값들의 평균을 계산합니다.">거래당 평균 포인트</MetricLabel><strong>${format(overview.oneLotPnl ?? 0)}</strong></div><div className="average-term"><MetricLabel>평균 수익 PnL</MetricLabel><strong className="is-profit">+${format(Math.abs(overview.averageWin ?? 0))}</strong></div><div className="average-term"><MetricLabel>평균 손실 PnL</MetricLabel><strong className="is-loss">-${format(Math.abs(overview.averageLoss ?? 0))}</strong></div></article>; }
