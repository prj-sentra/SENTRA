...
function campaignHeadConfirmation(isHead: boolean): string {
  return isHead
    ? '이 첫 진입 지정을 해제하시겠습니까?\n해제하면 시간 간격 기준으로 자동 재분류됩니다.\n완료될 때까지 이 계정의 첫 진입 변경은 잠깁니다. API 변경 요청 후 매매를 다시 불러오며, 이 진입을 다시 선택합니다.'
    : '이 진입을 첫 진입으로 지정하시겠습니까?\n이 진입부터 별도 매매로 분할됩니다.\n다른 매매는 시간 간격 기준으로 자동 재분류될 수 있습니다.\n완료될 때까지 이 계정의 첫 진입 변경은 잠깁니다. API 변경 요청 후 매매를 다시 불러오며, 이 진입을 다시 선택합니다.';
}
...
export function ExecutionTradeRow({ trade, campaign, busy = false, onChangeCampaignHead }: ExecutionTradeRowProps) {
  ...
  return <article className="execution-row">
    <div className="execution-time-row">
      ...
      {isHead ? <span className="campaign-head-status">{campaign?.headSource === 'MANUAL' ? '첫 진입 · 수동 지정' : '첫 진입 · 자동 지정'}</span> : null}
      ...
      }}>{canUnset ? '지정 해제' : '첫 진입으로 지정'}</button> : null}
    </div>
